...
CLEAN_DATA_LABEL = ''
CLEAN_DATA_FILENAME = 'coil_2000.npz'
...
def process_coil_2000():
    ...
    #TODO:
    """
    data_path = get_raw_dataset_home(RAW_DATA_LABEL)
    parse = lambda src: np.loadtxt(join(data_path, src), dtype=int)
...
def get_clean_dataset_path():
    return join(get_clean_dataset_home(CLEAN_DATA_LABEL),
                CLEAN_DATA_FILENAME)
...
def convert_coil_2000():
    ...
    np.savez(get_clean_dataset_path, data=d, label=l)
